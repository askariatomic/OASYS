...
$query_one = "SELECT KD_SURAT, NOMOR_SURAT, PERIHAL, TANGGAL FROM `oasys_surat_internal` WHERE NIP = '".$_SESSION['nip']."' ORDER BY TANGGAL DESC";
...
<html>
	...
	<body>
	  <h3>Daftar Surat Keluar Internal</h3>
	  ...
	  <?php foreach ($data_one as $a){ ?>
	    <tr>
		  <td><?php echo $data_one->getString('NOMOR_SURAT');  ?></td>
		  ...
		  <td><a target="_blank" href="viewSuratKeluarInternal?kd_surat=<?php echo $data_one->getString('KD_SURAT') ?>">Lihat</a></td>		  
	    </tr>
	  <?php } ?>
	   </tbody>
	  </table>
