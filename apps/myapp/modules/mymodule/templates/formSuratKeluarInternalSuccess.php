...
<html>
	<head>
	  ...
	  <script type="text/javascript">
	    ...
	    <?php 
		$query_six = "SELECT CONCAT(KD_KLASIFIKASI, ' - ', KLASIFIKASI) AS `KLS`, KD_KLASIFIKASI FROM `oasys_klasifikasi` WHERE KATEGORI = '".$kategori."'";
		$statement = $connection->prepareStatement($query_six);
		$data_six = $statement->executeQuery();
		?>
	    addOptionsTextValue(<?php foreach ($data_six as $a){ ?>"<?php echo $data_six->getString('KLS'); ?>", "<?php echo $data_six->getString('KD_KLASIFIKASI'); ?>", <?php } ?>"","");
	    <?php } ?>
	  </script>
      ...
      <form name="formSuratKeluarInternal" method="post" enctype="multipart/form-data">
		<table>
		  ...
		  <tr>
		    <td></td>
    	    <td></td>
    	    <td><select multiple="multiple" name="tembusan[]" id="tembusan"></select><input type="button"  id="remove_tembusan" value="Remove" />
		  </tr>
		  <tr>
		  ...
		  <tr>
			<td></td>
			<td></td>
		    <td><input type="button" name="kirim" value="Kirim" onclick="Kirim();" /><input type="button" name="simpan" value="Simpan" onclick="Simpan();" /></td>
		  </tr>
		</table>
	  </form>
	</body>
